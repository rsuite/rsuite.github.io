...
const readfile = path =>
  fs.readFileSync(resolve(__dirname, path), { encoding: 'UTF-8' });
...
const themeArtsboardData = Object.fromEntries(
  themes.map(theme => {
    const deafaultThemeDesignHtmlData = readfile(
      `${DESIGN_INDEX_PATH}/${theme}/index.html`
    );
    const defaultJsonData = gerJSONDataFromDesignHtmlData(
      deafaultThemeDesignHtmlData
    );
